...
#endif
#ifdef CS333_P4
//These two are the functions I will call in proc.c
...
sys_setpriority(void)
{
  ...
  if(priority > MAXPRIO)
  {
    cprintf("Cannot have a prioity higher than %d\n",MAXPRIO);
    return -1;
  }
  if(priority < 0)
  {
    cprintf("You cannot have a priority less than zero.\n");
    return -1;
  }
  value_returned = set_priority(pid,priority);
  if(value_returned == -1)
  {
  ...
  if(value_returned == -2)
  {
    cprintf("Position not changed, already at that priority.\n");
  }
  cprintf("PRIORITY FOR PID %d set to %d.\n",pid,priority);
